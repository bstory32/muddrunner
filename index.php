<?php include ('layout/header.php') ?>



<!-- start of events section -->

<div class="upcoming-events">
  <div class="container">
    <div class="row">

      <div class="col-sm-6 event-section">
        <div class="event-header">
          <h2>UPCOMING EVENTS</h2>
        </div>
        <ul class="event-list">
          <li>
            <img src="public/images/hennepin_banner1.jpg">
            <span>September 12 &amp 13, 2015</span>
            <p>The Hennepin Hundred</p>
            <p class="coming-soon">Registration Open Nov. 28</p>
          </li>
          <li>
            <img src="public/images/clean_up_2.jpg">
            <span>September 20, 2014</span>
            <p>Trail Clean Up - Sternes Woods</p>
            <!-- <a href="http://ultrasignup.com/">Register >></a> -->

            <p class="coming-soon">Registration Coming Soon</p>
          </li>
          <li>
            <img src="public/images/photos/event4.jpg" alt="">
            <span>January 10, 2015</span>
            <p>Frozen Gnome 50k/10k</p>
            <!-- <a href="http://ultrasignup.com/">Register >></a> -->
            <p class="coming-soon">Registration Coming Soon</p>

          </li>
          
          <li>
            <img src="public/images/photos/event3.jpg">
            <span>April 18, 2015</span>
            <p>Earth Day 50k/15k/5k</p>
            <!-- <a href="http://ultrasignup.com/">Register >></a> -->
            <p class="coming-soon">Registration Coming Soon</p>
          </li>
          
        </ul>
        <!-- <div class="more">
          <a href="http://ultrasignup.com/">View all Events >></a>
        </div> -->
      </div>

      <div class="col-sm-6">
        <div class="main-message">
          <h1>I survived Frozen Gnome.</h1>
          <hr>
          <!-- <img src="public/images/splatter.png" alt=""> -->
          <p>Photographed at <strong>Frozen Gnome</strong> by <em>Scott Laudick</em></p>
        </div>
      </div>
    </div>
  </div>
</div>
<!-- end of events section -->

<!-- start of featured obstacle -->

<div class="background-image">
  <div class="container">
    <div class="row">
      <div class="col-sm-12 sponsors-section">
        <div class="sponsors-info">
          <h1>Our Sponsors</h1>
        </div>
        <div class="sponsors-details">
          <ul>
            <li><!-- <a href="#"> --><img src="public/images/sponsors/inov.png" alt=""><!-- </a> --></li>
            <li><!-- <a href="#"> --><img src="public/images/sponsors/enjoy-illinois.png" alt=""><!-- </a> --></li>
            <li><!-- <a href="#"> --><img src="public/images/sponsors/running-depot.png" alt=""><!-- </a> --></li>
            <li><!-- <a href="#"> --><img src="public/images/sponsors/trails-for-illinois.png" alt=""><!-- </a> --></li>
          </ul>
        </div>
      </div>
    </div>
  </div>
</div>

<!-- end of featured of obstacle -->

<!-- start of news section -->

<!-- <div class="background2">
  <div class="container">
    <div class="row">
      <div class="col-sm-6 news-section">
        <div class="news-header">
          <h2>News</h2>
        </div>
        <ul class="articles">
          <li>
            <img src="http://placehold.it/140x100">
            <p>Meet the Muddrunner Founder</p>
            <a href="#">Read Article >></a>
          </li>

          <li>
            <img src="http://placehold.it/140x100">
            <p>Are you tough enough?</p>
            <a href="#">Read Article >></a>
          </li>

          <li>
            <img src="http://placehold.it/140x100">
            <p>Tank Away: Mudder Tough</p>
            <a href="#">Read Article >></a>
          </li>
        </ul>

        <div class="more">
          <a href="#">See all news >></a>
        </div>
      </div> -->

      <!-- end of new section -->

      <!-- start of projects section -->

      <!-- <div class="col-sm-6 more-info">
        <div class="projects">
          <h3>Mudd Project</h3>
          <hr class="blue">
          <img src="http://placehold.it/140x100">
          <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Quas quia quisquam enim atque totam cupiditate quaerat ducimus provident deserunt, hic reiciendis adipisci suscipit aperiam.</p>

        </div>
        <div class="more-projects">
          <a href="#">Learn More >></a>
        </div>
        <div class="projects">
          <h3>Train Tough</h3>
          <hr class="blue">
          <img src="http://placehold.it/140x100">
          <p>Lorem ipsum dolor commodi, dolorum repellat maxime culpa fugit sunt omnis exercitationem id possimus dolore at ab porro.</p>

        </div>
        <div class="more-projects">
          <a href="#">Shop More >></a>
        </div>
      </div>
    </div>
  </div>
</div> -->

<!-- end of projects section -->

<?php include ('layout/footer.php') ?>


</body>
</html>